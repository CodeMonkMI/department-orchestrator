...
export class RoomController {
  ...
  async find(_req: Request, res: Response, _next: NextFunction) {
    const data = await this.roomService.find();
    return res.status(200).json(data);
  }

  @GET("/:id")
  async single(req: Request, res: Response, _next: NextFunction) {
    const data = await this.roomService.findByID(req.params.id);
    return res.status(200).json(data);
  }
  ...
  async create(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const newData = await this.roomService.create(parsedData.data);
  ...
  async remove(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const findData = await this.roomService.findByID(id);
      if (!findData)
        throw this.roomService.generateError(
...
export const RoomControllerToken = Symbol("RoomControllerToken");
